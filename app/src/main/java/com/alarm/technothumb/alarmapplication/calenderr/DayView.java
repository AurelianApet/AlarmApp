...
public class DayView extends View implements View.OnCreateContextMenuListener,
    ...
    private static final String[] CALENDARS_PROJECTION = new String[]{
            ...
            CalendarContract.Calendars.CALENDAR_ACCESS_LEVEL, // 1
    ...
    private static final String CALENDARS_WHERE = CalendarContract.Calendars._ID + "=%d";
    ...
    private final Runnable mClearClick = new Runnable() {
        ...
        public void run() {
            if (mClickedEvent != null) {
                mController.sendEventRelatedEvent(this, CalendarController.EventType.VIEW_EVENT, mClickedEvent.id,
    ...
    private  int getEventAccessLevel(Context context, Event e) {
        ...
        Cursor cursor = cr.query(ContentUris.withAppendedId(CalendarContract.Events.CONTENT_URI, e.id),
    ...
    public void updateTitle() {
        ...
        mController.sendEvent(this, CalendarController.EventType.UPDATE_TITLE, start, end, null, -1, CalendarController.ViewType.CURRENT,
        ...
        if (mNumDays > 1) {
            ...
            if (trackBallSelection) {
                if (selectedEvent == null) {
                    ...
                    mController.sendEventRelatedEventWithExtra(this, CalendarController.EventType.CREATE_EVENT, -1,
                    ...
                    mController.sendEventRelatedEvent(this, CalendarController.EventType.VIEW_EVENT, selectedEvent.id,
...
//                break;
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if ((selectionDay < mFirstJulianDay) || (selectionDay > mLastJulianDay)) {
            ...
            mController.sendEvent(this, CalendarController.EventType.GO_TO, date, end, -1, CalendarController.ViewType.CURRENT);
            return true;
        }
        if (mSelectionDay != selectionDay) {
            ...
            mController.sendEvent(this, CalendarController.EventType.GO_TO, date, date, -1, CalendarController.ViewType.CURRENT);
        }
        ...
        if (layout == null || r.width() != layout.getWidth()) {
            ...
            switch (event.selfAttendeeStatus) {
                ...
                case CalendarContract.Attendees.ATTENDEE_STATUS_DECLINED:
                    ...
                    break;
                case CalendarContract.Attendees.ATTENDEE_STATUS_NONE: // Your own events
    ...
    private void doSingleTapUp(MotionEvent ev) {
        ...
        if (!validPosition) {
            if (y < DAY_HEADER_HEIGHT) {
                ...
                mController.sendEvent(this, CalendarController.EventType.GO_TO, null, null, selectedTime, -1,
                        CalendarController.ViewType.DAY, CalendarController.EXTRA_GOTO_DATE, null, null);
            }
            return;
        }
        ...
        if (pressedSelected && mSavedClickedEvent == null) {
            ...
            mSelectionMode = SELECTION_SELECTED;
            mController.sendEvent(this, CalendarController.EventType.GO_TO, startTime, endTime, -1, CalendarController.ViewType.CURRENT,
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        if (which == 0) {
                            ...
                            mController.sendEventRelatedEventWithExtra(this,
                                    CalendarController.EventType.CREATE_EVENT, -1, getSelectedTimeInMillis(), 0, -1,
                                    -1, extraLong, -1);
                        }
                    }
    ...
    private class GotoBroadcaster implements Animation.AnimationListener {
        ...
        public void onAnimationEnd(Animation animation) {
            ...
            if (mCounter == sCounter) {
                mController.sendEvent(this, CalendarController.EventType.GO_TO, mStart, mEnd, null, -1,
        ...
        public boolean onMenuItemClick(MenuItem item) {
            switch (item.getItemId()) {
                case MENU_EVENT_VIEW: {
                    if (mSelectedEvent != null) {
                        mController.sendEventRelatedEvent(this, CalendarController.EventType.VIEW_EVENT_DETAILS,
                ...
                case MENU_EVENT_EDIT: {
                    if (mSelectedEvent != null) {
                        mController.sendEventRelatedEvent(this, CalendarController.EventType.EDIT_EVENT,
                                mSelectedEvent.id, mSelectedEvent.startMillis,
                                mSelectedEvent.endMillis, 0, 0, -1);
                    }
                    break;
                ...
                case MENU_DAY: {
                    mController.sendEvent(this, CalendarController.EventType.GO_TO, getSelectedTime(), null, -1,
                ...
                case MENU_EVENT_CREATE: {
                    long startMillis = getSelectedTimeInMillis();
                    long endMillis = startMillis + DateUtils.HOUR_IN_MILLIS;
                    mController.sendEventRelatedEvent(this, CalendarController.EventType.CREATE_EVENT, -1,
                ...
                case MENU_EVENT_DELETE: {
                    if (mSelectedEvent != null) {
                        ...
                        long end = selectedEvent.endMillis;
                        long id = selectedEvent.id;
                        mController.sendEventRelatedEvent(this, CalendarController.EventType.DELETE_EVENT, id, begin,
                                end, 0, 0, -1);
                    }
                    break;
                }
