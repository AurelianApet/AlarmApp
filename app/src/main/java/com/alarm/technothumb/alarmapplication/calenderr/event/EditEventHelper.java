...
public class EditEventHelper {
    ...
    // Visible for testing
    static final String ATTENDEES_DELETE_PREFIX = CalendarContract.Attendees.EVENT_ID + "=? AND "
            + CalendarContract.Attendees.ATTENDEE_EMAIL + " IN (";
    ...
    static final String CALENDARS_WHERE_WRITEABLE_VISIBLE = CalendarContract.Calendars.CALENDAR_ACCESS_LEVEL + ">="
    ...
    static final String ATTENDEES_WHERE = CalendarContract.Attendees.EVENT_ID + "=? AND attendeeEmail IS NOT NULL";
    ...
    public boolean saveEvent(CalendarEventModel model, CalendarEventModel originalModel,
        ...
        if (uri == null) {
            ...
            ContentProviderOperation.Builder b = ContentProviderOperation.newInsert(
                    CalendarContract.Events.CONTENT_URI).withValues(values);
            ops.add(b.build());
            ...
            values.put(CalendarContract.Events.ORIGINAL_SYNC_ID, originalModel.mSyncId);
            values.put(CalendarContract.Events.ORIGINAL_INSTANCE_TIME, begin);
            boolean allDay = originalModel.mAllDay;
            values.put(CalendarContract.Events.ORIGINAL_ALL_DAY, allDay ? 1 : 0);
            values.put(CalendarContract.Events.STATUS, originalModel.mEventStatus);
            ...
            if (TextUtils.isEmpty(model.mRrule)) {
                ...
                eventIdIndex = ops.size();
                values.put(CalendarContract.Events.STATUS, originalModel.mEventStatus);
                ops.add(ContentProviderOperation.newInsert(CalendarContract.Events.CONTENT_URI).withValues(values)
                        .build());
            } else {
                if (isFirstEventInSeries(model, originalModel)) {
                    checkTimeDependentFields(originalModel, model, values, modifyWhich);
                    ...
                    eventIdIndex = ops.size();
                    values.put(CalendarContract.Events.STATUS, originalModel.mEventStatus);
                    ops.add(ContentProviderOperation.newInsert(CalendarContract.Events.CONTENT_URI).withValues(
                            values).build());
                }
            }
            forceSaveReminders = true;
        ...
        if (hasAttendeeData && model.mOwnerAttendeeId == -1) {
            ...
            if (model.mAttendeesList.size() != 0 && Utils.isValidEmail(ownerEmail)) {
                ...
                values.clear();
                values.put(CalendarContract.Attendees.ATTENDEE_EMAIL, ownerEmail);
                ...
                if (newEvent) {
                    b = ContentProviderOperation.newInsert(CalendarContract.Attendees.CONTENT_URI)
                            .withValues(values);
                    b.withValueBackReference(CalendarContract.Attendees.EVENT_ID, eventIdIndex);
                } else {
                    values.put(CalendarContract.Attendees.EVENT_ID, model.mId);
                    b = ContentProviderOperation.newInsert(CalendarContract.Attendees.CONTENT_URI)
                            .withValues(values);
                }
                ops.add(b.build());
            ...
            values.clear();
            values.put(CalendarContract.Attendees.ATTENDEE_STATUS, model.mSelfAttendeeStatus);
            values.put(CalendarContract.Attendees.EVENT_ID, model.mId);
            ...
            if (newEvent || !TextUtils.equals(originalAttendeesString, attendees)) {
                ...
                if (!newEvent) {
                    ...
                    if (removedAttendees.size() > 0) {
                        b = ContentProviderOperation.newDelete(CalendarContract.Attendees.CONTENT_URI);
                ...
                if (newAttendees.size() > 0) {
                    ...
                    for (CalendarEventModel.Attendee attendee : newAttendees.values()) {
                        values.clear();
                        values.put(CalendarContract.Attendees.ATTENDEE_NAME, attendee.mName);
                        values.put(CalendarContract.Attendees.ATTENDEE_EMAIL, attendee.mEmail);
                        values.put(CalendarContract.Attendees.ATTENDEE_RELATIONSHIP,
                                CalendarContract.Attendees.RELATIONSHIP_ATTENDEE);
                        values.put(CalendarContract.Attendees.ATTENDEE_TYPE, CalendarContract.Attendees.TYPE_REQUIRED);
                        values.put(CalendarContract.Attendees.ATTENDEE_STATUS, CalendarContract.Attendees.ATTENDEE_STATUS_NONE);
    ...
    void checkTimeDependentFields(CalendarEventModel originalModel, CalendarEventModel model,
        ...
        if (oldBegin == newBegin && oldEnd == newEnd && oldAllDay == newAllDay
            ...
            values.remove(CalendarContract.Events.DTSTART);
            values.remove(CalendarContract.Events.DTEND);
            values.remove(CalendarContract.Events.DURATION);
            values.remove(CalendarContract.Events.ALL_DAY);
            values.remove(CalendarContract.Events.RRULE);
            values.remove(CalendarContract.Events.EVENT_TIMEZONE);
            return;
        }
        ...
        if (modifyWhich == MODIFY_ALL) {
            ...
            values.put(CalendarContract.Events.DTSTART, oldStartMillis);
        }
    }
    ...
    public static boolean saveReminders(ArrayList<ContentProviderOperation> ops, long eventId,
                                        ArrayList<CalendarEventModel.ReminderEntry> reminders, ArrayList<CalendarEventModel.ReminderEntry> originalReminders,
                                        boolean forceSave) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            values.clear();
            values.put(CalendarContract.Reminders.MINUTES, re.getMinutes());
            values.put(CalendarContract.Reminders.METHOD, re.getMethod());
            values.put(CalendarContract.Reminders.EVENT_ID, eventId);
            b = ContentProviderOperation.newInsert(CalendarContract.Reminders.CONTENT_URI).withValues(values);
            ops.add(b.build());
        }
        return true;
    }
    ...
    public static boolean saveRemindersWithBackRef(ArrayList<ContentProviderOperation> ops,
        ...
        ContentProviderOperation.Builder b = ContentProviderOperation
                .newDelete(CalendarContract.Reminders.CONTENT_URI);
        b.withSelection(CalendarContract.Reminders.EVENT_ID + "=?", new String[1]);
        ...
        for (int i = 0; i < len; i++) {
            ...
            values.put(CalendarContract.Reminders.METHOD, re.getMethod());
            b = ContentProviderOperation.newInsert(CalendarContract.Reminders.CONTENT_URI).withValues(values);
            b.withValueBackReference(CalendarContract.Reminders.EVENT_ID, eventIdIndex);
            ops.add(b.build());
        }
        return true;
    }
    ...
    public static boolean canModifyCalendar(CalendarEventModel model) {
        return model.mCalendarAccessLevel >= CalendarContract.Calendars.CAL_ACCESS_CONTRIBUTOR
                || model.mCalendarId == -1;
    }

    public static boolean canAddReminders(CalendarEventModel model) {
        return model.mCalendarAccessLevel >= CalendarContract.Calendars.CAL_ACCESS_READ;
    }
