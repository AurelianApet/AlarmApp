...
import com.alarm.technothumb.alarmapplication.calenderr.EmailAddressAdapter;
import com.alarm.technothumb.alarmapplication.calenderr.EventInfoFragment;
import com.alarm.technothumb.alarmapplication.calenderr.EventRecurrenceFormatter;
import com.alarm.technothumb.alarmapplication.calenderr.GeneralPreferences;
import com.alarm.technothumb.alarmapplication.calenderr.RecipientAdapter;
import com.alarm.technothumb.alarmapplication.calenderr.Rfc822InputFilter;
import com.alarm.technothumb.alarmapplication.calenderr.Rfc822Validator;
import com.alarm.technothumb.alarmapplication.calenderr.Utils;
import com.alarm.technothumb.alarmapplication.calenderr.recurrencepicker.RecurrencePickerDialog;
...
//    View mColorPickerExistingEvent;
    ...
    public static  int selectDate, todayDate;
    /**
     ...
     * type constant (e.g. {@link CalendarContract.Events#AVAILABILITY_BUSY}) associated with the
    ...
    private ArrayList<CalendarEventModel.ReminderEntry> mUnsupportedReminders = new ArrayList<CalendarEventModel.ReminderEntry>();
    private String mRrule, formattedDate;

    public EditEventView(Activity activity, View view, EditEventHelper.EditDoneRunnable done,
...
//        mColorPickerExistingEvent = view.findViewById(R.id.change_color_existing_event);
      ...
      //  mEditOnlyList.add(view.findViewById(R.id.visibility_row));
        ...
        mStartTime1 = new Time(mTimezone);
         Log.e("today", mStartTime1+"");
    ...
    private void prepareReminders() {
        ...
        if (model.mHasAlarm) {
            ...
            for (CalendarEventModel.ReminderEntry re : reminders) {
                if (mReminderMethodValues.contains(re.getMethod())
                        || re.getMethod() == CalendarContract.Reminders.METHOD_DEFAULT) {
    ...
    private int findSelectedCalendarPosition(Cursor calendarsCursor, long calendarId) {
        ...
        int calendarIdColumn = calendarsCursor.getColumnIndexOrThrow(CalendarContract.Calendars._ID);
        int position = 0;
    ...
    private int findDefaultCalendarPosition(Cursor calendarsCursor) {
        ...
        int calendarsOwnerIndex = calendarsCursor.getColumnIndexOrThrow(CalendarContract.Calendars.OWNER_ACCOUNT);
        int accountNameIndex = calendarsCursor.getColumnIndexOrThrow(CalendarContract.Calendars.ACCOUNT_NAME);
        int accountTypeIndex = calendarsCursor.getColumnIndexOrThrow(CalendarContract.Calendars.ACCOUNT_TYPE);
        int position = 0;
        calendarsCursor.moveToPosition(-1);
        while (calendarsCursor.moveToNext()) {
    ...
    private void addReminder() {
        ...
        if (mDefaultReminderMinutes == GeneralPreferences.NO_REMINDER) {
            EventViewUtils.addReminder(mActivity, mScrollView, this, mReminderItems,
                    ...
                    CalendarEventModel.ReminderEntry.valueOf(GeneralPreferences.REMINDER_DEFAULT_TIME),
                    mModel.mCalendarMaxReminders, null);
        } else {
            EventViewUtils.addReminder(mActivity, mScrollView, this, mReminderItems,
                    mReminderMinuteValues, mReminderMinuteLabels,
                    mReminderMethodValues, mReminderMethodLabels,
                    CalendarEventModel.ReminderEntry.valueOf(mDefaultReminderMinutes),
                    mModel.mCalendarMaxReminders, null);
        }
        updateRemindersVisibility(mReminderItems.size());
        EventViewUtils.updateAddReminderButton(mView, mReminderItems, mModel.mCalendarMaxReminders);
    ...
    protected void setAllDayViewsVisibility(boolean isChecked) {
        if (isChecked) {
            if (mEndTime.hour == 0 && mEndTime.minute == 0) {
                ...
                // before the
                // start time.
                if (mStartTime.before(mStartTime1)) {
                   Toast.makeText(mActivity,"you can not select Old Date", Toast.LENGTH_SHORT).show();
                    mStartTime.set(mStartTime1);
                    endMillis = mStartTime.normalize(true);
                }
...
//    }
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
        ...
        int maxRemindersColumn = c.getColumnIndexOrThrow(CalendarContract.Calendars.MAX_REMINDERS);
        mModel.mCalendarMaxReminders = c.getInt(maxRemindersColumn);
        int allowedRemindersColumn = c.getColumnIndexOrThrow(CalendarContract.Calendars.ALLOWED_REMINDERS);
        mModel.mCalendarAllowedReminders = c.getString(allowedRemindersColumn);
        int allowedAttendeeTypesColumn = c.getColumnIndexOrThrow(CalendarContract.Calendars.ALLOWED_ATTENDEE_TYPES);
        mModel.mCalendarAllowedAttendeeTypes = c.getString(allowedAttendeeTypesColumn);
        int allowedAvailabilityColumn = c.getColumnIndexOrThrow(CalendarContract.Calendars.ALLOWED_AVAILABILITY);
    ...
    private class DateListener implements DatePickerDialog.OnDateSetListener {
        ...
        public void onDateSet(DatePickerDialog view, int year, int month, int monthDay) {
            int mewMonth = month+1;
            if(mewMonth<10){
                if(monthDay<10){
                    selectedStartDate = year+"0"+mewMonth+""+"0"+monthDay ;
                }else {
                    selectedStartDate = year+"0"+mewMonth+""+monthDay ;
                }

            }else {
                selectedStartDate = year+""+mewMonth+""+monthDay ;
            }
            Log.e(TAG, "onDateSet: " + year + " " + mewMonth + " " + monthDay);
            // Cache the member variables locally to avoid inner class overhead.
            Time startTime = mStartTime;
            Time startTime1 = mStartTime1;
            Time endTime = mEndTime;

            //String selectedStartDate = monthDay+""+mewMonth+""+year ;
            selectDate = Integer.parseInt(selectedStartDate);
            todayDate = Integer.parseInt(formattedDate);
            ...
            if (mView == mStartDateButton) {
                ...
                startMillis = startTime.normalize(true);
                Log.e("selectedStartDate", selectDate+"");
                Log.e("TodayDate", todayDate+"");
                Log.e("DateCondition", (todayDate <= selectDate)+"");
                ...
                if (selectDate < todayDate) {
                    ...
                    Toast.makeText(mActivity, "You can not add old Date", Toast.LENGTH_SHORT).show();
