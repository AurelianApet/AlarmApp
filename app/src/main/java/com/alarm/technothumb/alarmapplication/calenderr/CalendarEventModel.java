...
import com.alarm.technothumb.alarmapplication.calenderr.event.EventColorCache;
...
public class CalendarEventModel implements Serializable {
    ...
    public int mAvailability = CalendarContract.Events.AVAILABILITY_BUSY;
    ...
    public int mCalendarAccessLevel = CalendarContract.Calendars.CAL_ACCESS_CONTRIBUTOR;
    public int mEventStatus = CalendarContract.Events.STATUS_CONFIRMED;
    ...
    public CalendarEventModel(Context context, Intent intent) {
        ...
        String location = intent.getStringExtra(CalendarContract.Events.EVENT_LOCATION);
        if (location != null) {
            mLocation = location;
        }

        String description = intent.getStringExtra(CalendarContract.Events.DESCRIPTION);
        if (description != null) {
            mDescription = description;
        }

        int availability = intent.getIntExtra(CalendarContract.Events.AVAILABILITY, -1);
        if (availability != -1) {
            mAvailability = availability;
        }

        int accessLevel = intent.getIntExtra(CalendarContract.Events.ACCESS_LEVEL, -1);
        if (accessLevel != -1) {
            if (accessLevel > 0) {
        ...
        String rrule = intent.getStringExtra(CalendarContract.Events.RRULE);
    ...
    public void clear() {
        ...
        mAccessLevel = 0;
        mEventStatus = CalendarContract.Events.STATUS_CONFIRMED;
        mOrganizerCanRespond = false;
        mCalendarAccessLevel = CalendarContract.Calendars.CAL_ACCESS_CONTRIBUTOR;
    ...
    public static class Attendee implements Serializable {
        ...
        public Attendee(String name, String email) {
            this(name, email, CalendarContract.Attendees.ATTENDEE_STATUS_NONE, null, null);
        }
    ...
    public static class ReminderEntry implements Comparable<ReminderEntry>, Serializable {
         ...
         * @param method Type of alert ({@link CalendarContract.Reminders#METHOD_ALERT}, etc).
         */
        ...
        public static ReminderEntry valueOf(int minutes) {
            return valueOf(minutes, CalendarContract.Reminders.METHOD_DEFAULT);
        }
        ...
        public boolean equals(Object obj) {
            ...
            return re.mMethod == mMethod ||
                    (re.mMethod == CalendarContract.Reminders.METHOD_DEFAULT && mMethod == CalendarContract.Reminders.METHOD_ALERT) ||
                    (re.mMethod == CalendarContract.Reminders.METHOD_ALERT && mMethod == CalendarContract.Reminders.METHOD_DEFAULT);
        }
