...
public class AlarmScheduler {
    ...
    private static final String INSTANCES_WHERE = CalendarContract.Events.VISIBLE + "=? AND "
            + CalendarContract.Instances.BEGIN + ">=? AND " + CalendarContract.Instances.BEGIN + "<=? AND "
    ...
    private static final String REMINDERS_WHERE = CalendarContract.Reminders.METHOD + "=1 AND "
            + CalendarContract.Reminders.EVENT_ID + " IN ";
    ...
    private static void queryNextReminderAndSchedule(Cursor instancesCursor, Context context,
        ...
        while (!instancesCursor.isAfterLast()) {
            ...
            Cursor cursor = null;
            try {
                cursor = contentResolver.query(CalendarContract.Reminders.CONTENT_URI, REMINDERS_PROJECTION,
