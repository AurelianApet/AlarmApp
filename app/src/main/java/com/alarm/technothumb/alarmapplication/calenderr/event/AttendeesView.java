...
import com.alarm.technothumb.alarmapplication.calenderr.ContactsAsyncHelper;
...
public class AttendeesView extends LinearLayout implements View.OnClickListener {
    ...
    private static final String[] PROJECTION = new String[] {
            ContactsContract.RawContacts.CONTACT_ID,     // 0
    ...
    private View updateAttendeeView(EditEventHelper.AttendeeItem item) {
        final CalendarEventModel.Attendee attendee = item.mAttendee;
        ...
        if (item.mAttendee.mStatus == CalendarContract.Attendees.ATTENDEE_STATUS_NONE) {
            item.mBadge.setAlpha(mNoResponsePhotoAlpha);
        } else {
            item.mBadge.setAlpha(mDefaultPhotoAlpha);
        }
        if (item.mAttendee.mStatus == CalendarContract.Attendees.ATTENDEE_STATUS_DECLINED) {
    ...
    private void addOneAttendee(CalendarEventModel.Attendee attendee) {
        ...
        final EditEventHelper.AttendeeItem item = new EditEventHelper.AttendeeItem(attendee, mDefaultBadge);
        ...
        if (attendee.mIdentity != null && attendee.mIdNamespace != null) {
            ...
            selection = ContactsContract.Data.MIMETYPE + "=? AND " + ContactsContract.CommonDataKinds.Identity.IDENTITY + "=? AND " +
                    ContactsContract.CommonDataKinds.Identity.NAMESPACE + "=?";
            selectionArgs = new String[] {ContactsContract.CommonDataKinds.Identity.CONTENT_ITEM_TYPE, attendee.mIdentity,
    ...
    public void addAttendees(ArrayList<CalendarEventModel.Attendee> attendees) {
        synchronized (this) {
            for (final CalendarEventModel.Attendee attendee : attendees) {
                addOneAttendee(attendee);
            }
        }
    }

    public void addAttendees(HashMap<String, CalendarEventModel.Attendee> attendees) {
        synchronized (this) {
            for (final CalendarEventModel.Attendee attendee : attendees.values()) {
                addOneAttendee(attendee);
            }
        }
    }
    ...
    public void addAttendees(String attendees) {
        ...
        synchronized (this) {
            for (final Rfc822Token address : addresses) {
                final CalendarEventModel.Attendee attendee = new CalendarEventModel.Attendee(address.getName(), address.getAddress());
    ...
    public boolean isMarkAsRemoved(int index) {
        ...
        return ((EditEventHelper.AttendeeItem) view.getTag()).mRemoved;
    }

    public CalendarEventModel.Attendee getItem(int index) {
        final View view = getChildAt(index);
        if (view instanceof TextView) { // divider
            return null;
        }
        return ((EditEventHelper.AttendeeItem) view.getTag()).mAttendee;
    }

    @Override
    public void onClick(View view) {
        // Button corresponding to R.id.contact_remove.
        final EditEventHelper.AttendeeItem item = (EditEventHelper.AttendeeItem) view.getTag();
