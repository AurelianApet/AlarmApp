...
import com.alarm.technothumb.alarmapplication.calenderr.OtherPreferences;
...
public class AlertService extends Service {
    ...
    static final String[] ALERT_PROJECTION = new String[]{
            ...
            CalendarContract.CalendarAlerts.EVENT_ID,                // 1
            CalendarContract.CalendarAlerts.STATE,                   // 2
            CalendarContract.CalendarAlerts.TITLE,                   // 3
            CalendarContract.CalendarAlerts.EVENT_LOCATION,          // 4
            CalendarContract.CalendarAlerts.SELF_ATTENDEE_STATUS,    // 5
            CalendarContract.CalendarAlerts.ALL_DAY,                 // 6
            CalendarContract.CalendarAlerts.ALARM_TIME,              // 7
            CalendarContract.CalendarAlerts.MINUTES,                 // 8
            CalendarContract.CalendarAlerts.BEGIN,                   // 9
            CalendarContract.CalendarAlerts.END,                     // 10
            CalendarContract.CalendarAlerts.DESCRIPTION,             // 11
    };
    ...
    private static final String ACTIVE_ALERTS_SELECTION = "(" + CalendarContract.CalendarAlerts.STATE + "=? OR "
            + CalendarContract.CalendarAlerts.STATE + "=?) AND " + CalendarContract.CalendarAlerts.ALARM_TIME + "<=";
    private static final String[] ACTIVE_ALERTS_SELECTION_ARGS = new String[]{
            ...
            Integer.toString(CalendarContract.CalendarAlerts.STATE_SCHEDULED)
    };
    private static final String ACTIVE_ALERTS_SORT = "begin DESC, end DESC";
    private static final String DISMISS_OLD_SELECTION = CalendarContract.CalendarAlerts.END + "<? AND "
    ...
    static void dismissOldAlerts(Context context) {
        ...
        vals.put(CalendarContract.CalendarAlerts.STATE, CalendarContract.CalendarAlerts.STATE_DISMISSED);
        if (ActivityCompat.checkSelfPermission(context, Manifest.permission.WRITE_CALENDAR) != PackageManager.PERMISSION_GRANTED) {

            return;
        }
        cr.update(CalendarContract.CalendarAlerts.CONTENT_URI, vals, DISMISS_OLD_SELECTION, new String[]{
                Long.toString(currentTime), Integer.toString(CalendarContract.CalendarAlerts.STATE_SCHEDULED)
        });
    }
    ...
    static boolean updateAlertNotification(Context context) {
        ...
        Cursor alertCursor = cr.query(CalendarContract.CalendarAlerts.CONTENT_URI, ALERT_PROJECTION,
    ...
    static int processQuery(final Cursor alertCursor, final Context context,
        ...
        try {
            while (alertCursor.moveToNext()) {
                ...
                final boolean declined = status == CalendarContract.Attendees.ATTENDEE_STATUS_DECLINED;
                final boolean responded = status != CalendarContract.Attendees.ATTENDEE_STATUS_NONE
                ...
                final Uri alertUri = ContentUris
                        .withAppendedId(CalendarContract.CalendarAlerts.CONTENT_URI, alertId);
                ...
                if (sendAlert) {
                    if (state == CalendarContract.CalendarAlerts.STATE_SCHEDULED || newAlertOverride) {
                        newState = CalendarContract.CalendarAlerts.STATE_FIRED;
                        numFired++;
                        ...
                        // missed or delayed.
                        values.put(CalendarContract.CalendarAlerts.RECEIVED_TIME, currentTime);
                    }
                } else {
                    newState = CalendarContract.CalendarAlerts.STATE_DISMISSED;
                }

                // Update row if state changed
                if (newState != -1) {
                    values.put(CalendarContract.CalendarAlerts.STATE, newState);
                    state = newState;
                ...
                if (state == CalendarContract.CalendarAlerts.STATE_FIRED) {
                    // Record the time posting to notification manager.
                    // This is used for debugging missed alarms.
                    values.put(CalendarContract.CalendarAlerts.NOTIFY_TIME, currentTime);
                }
    ...
    private static final void rescheduleMissedAlarms(ContentResolver cr, Context context,
        ...
        Cursor cursor = cr.query(CalendarContract.CalendarAlerts.CONTENT_URI, projection,
    ...
    void processMessage(Message msg) {
        ...
        if (DEBUG) {
            Log.d(TAG, bundle.getLong(CalendarContract.CalendarAlerts.ALARM_TIME)
                    + " Action = " + action);
        }
