...
import com.alarm.technothumb.alarmapplication.calenderr.selectcalendars.SelectVisibleCalendarsActivity;
...
public class CalendarController {
    ...
    public void sendEventRelatedEvent(Object sender, long eventType, long eventId, long startMillis,
        ...
        sendEventRelatedEventWithExtra(sender, eventType, eventId, startMillis, endMillis, x, y,
                EventInfo.buildViewExtraLong(CalendarContract.Attendees.ATTENDEE_STATUS_NONE, false),
                selectedMillis);
    }
        ...
        synchronized (this) {
            ...
            if (mDispatchInProgressCounter == 0) {
                ...
                if (mToBeAddedEventHandlers.size() > 0) {
                    for (Map.Entry<Integer, EventHandler> food : mToBeAddedEventHandlers.entrySet()) {
    ...
    public void launchViewEvent(long eventId, long startMillis, long endMillis, int response) {
        ...
        intent.setClass(mContext, CalenderrActivity.class);
        intent.putExtra(EXTRA_EVENT_BEGIN_TIME, startMillis);
        intent.putExtra(EXTRA_EVENT_END_TIME, endMillis);
     ...
     //   SearchableInfo searchableInfo = searchManager.getSearchableInfo(componentName);
      //  SearchableInfo searchableInfo = searchManager.getSearchableInfo(componentName);
       SearchableInfo searchableInfo = searchManager.getSearchableInfo(new ComponentName(mContext, SearchActivity.class));
        Log.e("serchInfo", searchableInfo+"");
        Log.e("componentName", componentName+"");
        Log.e("query", query+"");

        if (searchableInfo.getSearchActivity() == null)
        {
            Log.e("null", "searchableInfo null");
        }
