...
public class EditEventFragment extends Fragment implements CalendarController.EventHandler, ColorPickerSwatch.OnColorSelectedListener {
    ...
    public EditEventFragment(CalendarController.EventInfo event, ArrayList<CalendarEventModel.ReminderEntry> reminders,
    ...
    private void startQuery() {
        ...
        if (mEvent != null) {
            if (mEvent.id != -1) {
                ...
                mUri = ContentUris.withAppendedId(CalendarContract.Events.CONTENT_URI, mEvent.id);
            } else {
                // New event. All day?
            ...
            if (mEventBundle.id != -1) {
                ...
                mUri = ContentUris.withAppendedId(CalendarContract.Events.CONTENT_URI, mEventBundle.id);
            }
        ...
        if (!newEvent) {
            ...
            mModel.mSelfAttendeeStatus = CalendarContract.Attendees.ATTENDEE_STATUS_ACCEPTED;
...
//        mContext.requestWindowFeature(Window.FEATURE_INDETERMINATE_PROGRESS);
    ...
    private boolean onActionBarItemSelected(int itemId) {
        if (itemId == R.id.action_done) {
            ...
            if (EditEventView.selectDate  <  EditEventView.todayDate) {

                Toast.makeText(getActivity(), "You can not add old Date", Toast.LENGTH_SHORT).show();
            }else {
